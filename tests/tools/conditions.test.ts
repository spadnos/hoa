...
test('add_condition creates a condition for a project', async () => {
  const result = await addCondition({ project_id: '2026-001', description: 'Submit revised plans' }, db);
  expect(result).toMatchObject({ project_id: '2026-001' });
  const conds = await listConditions({ project_id: '2026-001' }, db);
...
test('add_condition returns error for unknown project', async () => {
  const result = await addCondition({ project_id: '9999-999', description: 'x' }, db);
...
test('update_condition marks condition as satisfied', async () => {
  ...
  await updateCondition({ id: added.id, satisfied: true }, db);
  const conds = await listConditions({ project_id: '2026-001' }, db);
...
test('update_condition clears satisfied_at when satisfied=false', async () => {
  ...
  await updateCondition({ id: added.id, satisfied: true }, db);
  await updateCondition({ id: added.id, satisfied: false }, db);
...
test('list_conditions unsatisfied_only filters correctly', async () => {
  const c1 = (await addCondition({ project_id: '2026-001', description: 'First' }, db)) as {
    id: number;
    project_id: string;
  };
  await addCondition({ project_id: '2026-001', description: 'Second' }, db);
  await updateCondition({ id: c1.id, satisfied: true }, db);

  const all = await listConditions({ project_id: '2026-001' }, db);
  const unsatisfied = await listConditions({ project_id: '2026-001', unsatisfied_only: true }, db);
